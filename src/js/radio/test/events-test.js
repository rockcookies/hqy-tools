...
QUnit.test('callback list is not altered during trigger', function(assert) {
	assert.expect(2);
	var counter = 0, obj = _.extend({}, EventsObj);
...
QUnit.test('once variant one', function(assert) {
	...
	var a = _.extend({}, EventsObj).once('event', f);
	var b = _.extend({}, EventsObj).on('event', f);
...
QUnit.test('once with asynchronous events', function(assert) {
	...
	var obj = _.extend({}, EventsObj).once('async', func);
...
QUnit.test('once without a callback is a noop', function(assert) {
	assert.expect(0);
	_.extend({}, EventsObj).once('event').trigger('event');
});

QUnit.test('event functions are chainable', function(assert) {
	var obj = _.extend({}, EventsObj);
	var obj2 = _.extend({}, EventsObj);
	var fn = function() {};
...
QUnit.test('mixTo Class function', function(assert) {
	var counter = 0;
	function F() {
		this.counter = 0;
	};
...
QUnit.test('trigger arguments', function(assert) {
	var obj = new Events();
	var arrA;
	var arrB;
	...
	obj.on('all', function () {
		arrB = Array.prototype.slice.call(arguments);
	});

	obj.trigger('a', 1, 2, 3);

	assert.strictEqual(arrayEquals(arrA, [1, 2, 3]), true);
	assert.strictEqual(arrayEquals(arrB, ['a', 1, 2, 3]), true);
});
